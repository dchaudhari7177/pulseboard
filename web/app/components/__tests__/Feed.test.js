...
describe("Feed - clear all filters", () => {
  ...
  // getByRole, not getAllByRole(...)[0]: the toolbar control's name is
  // distinct from the empty state's "Clear filters", so this resolves to
  // exactly one element and fails loudly if that ever stops being true.
  const clearButton = () =>
